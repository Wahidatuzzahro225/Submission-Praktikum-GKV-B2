...
auto_array(const auto_array<T> &aarray) { array = aarray.array; 
...
void operator=(const auto_array<T> &aarray) { if (!isReleased && array != NULL) { 
delete[] array; 
} 
array = aarray.array; 
isReleased = aarray.isReleased; 
...
//Membaca data 
int bytesPerRow = ((width * 3 + 3) / 4) * 4 - (width * 3 % 4); int size = bytesPerRow * height; 
auto_array<char> pixels(new char[size]); 
input.seekg(dataOffset, ios_base::beg); 
input.read(pixels.get(), size); 
//Mengambil data yang mempunyai format benar 
auto_array<char> pixels2(new char[width * height * 3]); for(int y = 0; y < height; y++) { 
